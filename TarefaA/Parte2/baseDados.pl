/*prato(Nome, Restaurante, Preco, Tipo, Gastronomia, SensibilidadePreco, Contexto, idade)*/
prato('Seitan salteado com bechamel de alho frances', 'Alfacinha', 5.5, 'Vegetariano', vegetariano, [preco, custo_beneficio, qualidade], [jantar_amigos, jantar_romantico], [faixa16_35, faixa36_60, faixa61_mais]).
prato('Feijao a Moda do Porto com Arroz Integral', 'Semente Art Coffee & Plant Based Food', 9.5, 'Vegan', vegan, [preco, custo_beneficio, qualidade], [jantar_amigos, jantar_romantico], [faixa16_35, faixa36_60, faixa61_mais]).
prato('Fitoque - bife de frango', 'Eat Fit', 9.8, 'Low Carbs', baixo_calorias, [preco, custo_beneficio, qualidade], [jantar_amigos, jantar_romantico], [faixa16_35, faixa36_60, faixa61_mais]).
prato('Arroz de Pato', 'Continente' , 2.99 , 'Regional', regional, [preco], [jantar_amigos], [faixa16_35]).
prato('Tabua de Salmao', 'Cafe do Luis' , 5.5 , 'Regional', regional, [preco], [jantar_romantico] , [faixa16_35, faixa36_60]).
prato('Francesinha Especial de Bife de Novilho', 'Taberna Londrina', 12 , 'Regional', regional, [custo_beneficio, qualidade], [jantar_amigos, jantar_romantico], [faixa16_35, faixa36_60]).
prato('Bacalhau Espiritual', 'Gato do Rio', 14.9, 'Regional', regional, [custo_beneficio, qualidade], [jantar_amigos, jantar_romantico], [faixa36_60, faixa61_mais]).
prato('Rojoes com batata frita ao cubo', 'Arte na Brasa 2', 6.9, 'Regional', regional, [preco], [jantar_amigos, jantar_romantico], [faixa36_60, faixa61_mais]).
prato('Noodles', 'Otsu Biru', 5.5, 'Comida japonesa', oriental, [preco], [jantar_amigos], [faixa16_35, faixa36_60, faixa61_mais]).
prato('Temaki de Salmao', 'Otsu Biru', 5.5, 'Comida Japonesa', oriental, [preco], [jantar_romantico], [faixa16_35, faixa36_60, faixa61_mais]).
prato('Mix sushi to sashimi', 'Alma dEca', 40, 'Comida Japonesa', oriental, [qualidade], [jantar_romantico], [faixa16_35, faixa36_60, faixa61_mais]).
prato('Hot Sushi de Gambas', 'Sushi Em Tua Casa', 12 , 'Comida Japonesa', oriental, [custo_beneficio], [jantar_amigos], [faixa16_35, 35_60, faixa61_mais]).
prato('Big Mac', 'McDonalds', 5.9, 'Fast Food', fast_food, [preco], [jantar_amigos], [faixa16_35]).
prato('Balde Frango Duo 2', 'KFC', 7.80, 'Fast Food', fast_food, [preco], [jantar_romantico], [faixa16_35]).
prato('Hamburguer Tripeiro', 'Degema', 10.05, 'Fast Food', fast_food, [custo_beneficio], [jantar_amigos, jantar_romantico], [faixa16_35, faixa36_60]).
prato('Pizza Di Modena', 'Il fiume', 12.5, 'Fast Food', fast_food, [qualidade], [jantar_amigos, jantar_romantico], [faixa16_35, faixa36_60]).
prato('Wrap Frango e Cogumelos', 'Companhia', 8.3, 'Fast Food', fast_food, [preco], [jantar_amigos,jantar_romantico], [faixa36_60]).
prato('Pizza Norvegia', 'Il fiume', 18.9, 'Fast Food', fast_food, [qualidade], [jantar_amigos,jantar_romantico], [faixa61_mais]).
prato('Chove Canivetes', 'Degema', 8.65, 'Fast Food', fast_food, [custo_beneficio], [jantar_amigos], [faixa61_mais]).
prato('Wrap de Presunto e Brie', 'Companhia', 8.3, 'Fast Food' ,fast_food, [custo_beneficio], [jantar_romantico], [faixa61_mais]).
prato('Frango na Brasa', 'Tourigalo', 7, 'Fast Food', fast_food, [preco], [jantar_amigos, jantar_romantico], [faixa61_mais]).
prato('Lasanha alla Bolognese', 'Il fiume', 9.95, 'Comida Italiana', internacional, [custo_beneficio], [jantar_amigos], [faixa16_35, faixa36_60, faixa61_mais]).
prato('Farfalle alla Campagna', 'Il fiume', 9.50, 'Comida Italiana', internacional, [custo_beneficio], [jantar_romantico], [faixa16_35, faixa36_60, faixa61_mais]).
prato('Kebab no prato', 'Pak Doner Kebab', 5.70, 'Comida turca', internacional, [preco], [jantar_amigos], [faixa16_35]).
prato('Schnitzel', 'Pretzel Bakery', 7.8, 'Comida Alema', internacional, [preco], [jantar_romantico], [faixa16_35]).
prato('Espetos argentinos com molho picante', 'Romando', 21.25, 'Comida Argentina', internacional, [qualidade], [jantar_amigos], [faixa16_35]).
prato('Risotto de cogumelos selvagens', 'Romando', 16.5 , 'Comida Italiana', internacional, [qualidade], [jantar_romantico], [faixa16_35, faixa36_60]).
prato('Moamba de Galinha', 'Kianda', 25, 'Comida Africana', internacional, [qualidade], [jantar_romantico, jantar_amigos], [faixa36_60]).
prato('Picanha na Brasa', 'Fogo de Chao', 20, 'Comida Brasileira', internacional, [qualidade], [jantar_amigos, jantar_romantico], [faixa16_35, faixa36_60]).
prato('Rucula Asparagi e Parmigiano', 'Mamma mia', 8.5, 'Comida Italiana', internacional, [preco], [jantar_romantico], [faixa36_60]).
prato('Sandes de Salmao', 'O Gilins', 7.5, 'Comida Irlandesa', internacional, [preco], [jantar_amigos], [faixa16_35, faixa36_60]).
prato('Soufle de robalo e gambas', 'Turismo', 22.5, 'Comida Francesa', internacional, [qualidade], [jantar_romantico], [faixa36_60, faixa61_mais]).
prato('Rosbife a inglesa', 'Tia Isabel', 19, 'Comida Inglesa', internacional, [qualidade], [jantar_amigos], [faixa36_60, faixa61_mais]).
prato('Crepe Scandinave', 'Crepes a Francesa', 7.5, 'Comida Francesa', internacional, [preco], [jantar_amigos, jantar_romantico], [faixa61_mais]).
prato('Crepe Raclette', 'Crepes a Francesa', 7.5, 'Comida Francesa', internacional, [preco], [jantar_amigos, jantar_romantico], [faixa61_mais]).

membro( X, [X|_] ).
membro( X, [_|R] ) :- membro( X, R ).

procurarPrato(A,B,C,D):- findall([Y,Z,T],(prato(Y,Z,T,_,A,E,F,G), membro(B,E), membro(C,F), membro(D,G)),H), darPrint(H).
procurarPratoDieta(A):- findall([Y,Z,T], prato(Y,Z,T,_,A,_,_,_),H), darPrint(H).

darPrint(X) :-
X = [A | B],
write('_______________________________________________________________________'),nl,
write(''),nl,
write('  Prato = '),
A = [C | D],
write(C),nl,
write('  Restaurante = '),
D = [E | F],
write(E),nl,
write('  Preco = '),
F = [G],
write(G),nl,
nl,
(is_empty(B) -> darPrint(B); write('_______________________________________________________________________')).


is_empty(List):- member(_,List).

perfil(1):- procurarPrato(regional,qualidade,jantar_romantico,faixa16_35).
perfil(2):- procurarPrato(regional,qualidade,jantar_amigos,faixa16_35).
perfil(3):- procurarPrato(regional,custo_beneficio,jantar_romantico,faixa16_35).
perfil(4):- procurarPrato(regional,custo_beneficio,jantar_amigos,faixa16_35).
perfil(5):- procurarPrato(regional,preco,jantar_romantico,faixa16_35).
perfil(6):- procurarPrato(regional,preco,jantar_amigos,faixa16_35).
perfil(7):- procurarPrato(oriental,_,_,faixa36_60).
perfil(12):- procurarPrato(oriental,_,_,faixa16_35).
perfil(13):- procurarPrato(fast_food,qualidade,jantar_romantico,faixa16_35).
perfil(14):- procurarPrato(fast_food,qualidade,jantar_amigos,faixa16_35).
perfil(15):- procurarPrato(fast_food,custo_beneficio,jantar_romantico,faixa16_35).
perfil(16):- procurarPrato(fast_food,custo_beneficio,jantar_amigos,faixa16_35).
perfil(17):- procurarPrato(fast_food,preco,jantar_romantico,faixa16_35).
perfil(18):- procurarPrato(fast_food,preco,jantar_amigos,faixa16_35).
perfil(19):- procurarPrato(internacional,qualidade,jantar_romantico,faixa16_35).
perfil(20):- procurarPrato(internacional,qualidade,jantar_amigos,faixa16_35).
perfil(21):- procurarPrato(internacional,custo_beneficio,jantar_romantico,faixa16_35).
perfil(22):- procurarPrato(internacional,custo_beneficio,jantar_amigos,faixa16_35).
perfil(23):- procurarPrato(internacional,preco,jantar_romantico,faixa16_35).
perfil(24):- procurarPrato(internacional,preco,jantar_amigos,faixa16_35).
perfil(25):- procurarPrato(regional, qualidade, jantar_romantico, faixa36_60).
perfil(26):- procurarPrato(regional, qualidade, jantar_amigos, faixa36_60).
perfil(27):- procurarPrato(regional, custo_beneficio, jantar_romantico, faixa36_60).
perfil(28):- procurarPrato(regional, custo_beneficio, jantar_amigos, faixa36_60).
perfil(29):- procurarPrato(regional, preco, jantar_romantico, faixa36_60).
perfil(30):- procurarPrato(regional, preco, jantar_amigos, faixa36_60).
perfil(37):- procurarPrato(fast_food, qualidade, jantar_romantico, faixa36_60).
perfil(38):- procurarPrato(fast_food, qualidade, jantar_amigos, faixa36_60).
perfil(39):- procurarPrato(fast_food, custo_beneficio, jantar_romantico, faixa36_60).
perfil(40):- procurarPrato(fast_food, custo_beneficio, jantar_amigos, faixa36_60).
perfil(41):- procurarPrato(fast_food, preco, jantar_romantico, faixa36_60).
perfil(42):- procurarPrato(fast_food, preco, jantar_amigos, faixa36_60).
perfil(43):- procurarPrato(internacional, qualidade, jantar_romantico, faixa36_60).
perfil(44):- procurarPrato(internacional, qualidade, jantar_amigos, faixa36_60).
perfil(45):- procurarPrato(internacional, custo_beneficio, jantar_romantico, faixa36_60).
perfil(46):- procurarPrato(internacional, custo_beneficio, jantar_amigos, faixa36_60).
perfil(47):- procurarPrato(internacional, preco, jantar_romantico, faixa36_60).
perfil(48):- procurarPrato(internacional, preco, jantar_amigos, faixa36_60).
perfil(73):- procurarPratoDieta(vegetariano).
perfil(74):- procurarPratoDieta(vegan).
perfil(75):- procurarPratoDieta(baixo_calorias).